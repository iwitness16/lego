...
export default function OrderConfirmedPage() {
  ...
  useEffect(() => {
    if (whatsappUrl && !redirected.current) {
      ...
      // Small delay so the confirmed page renders first
      const t = setTimeout(() => {
    ...
    <div className="flex min-h-[80vh] items-center justify-center bg-paper px-4 py-16">
      <div className="w-full max-w-lg border border-line bg-surface shadow-card text-center">
        ...
        <div className="px-8 py-8 flex flex-col gap-6">
          ...
          {/* WhatsApp CTA (fallback if auto-open was blocked) */}
          {whatsappUrl && (
            <a
              href={whatsappUrl}
              target="_blank"
          ...
          <div className="flex gap-3">
            <Link href="/sets" className="flex-1 border border-line py-2.5 text-center text-sm font-semibold text-ink hover:bg-paper transition-colors">
              Continue shopping
            </Link>
            <Link href="/" className="flex-1 border border-ink bg-ink py-2.5 text-center text-sm font-semibold text-white hover:bg-brand-700 transition-colors">
              Home
            </Link>
          </div>
        </div>
